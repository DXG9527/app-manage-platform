...
export default class UmdDesignList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            pageDesign: [],
    ...
    onDeleteClick = (event, designId) => {
        axiosConfig.delete(`/umd/config/${designId}`)
            .then((result) => {
                message.destroy();
    ...
    onEditClick = (event, designId) => {
        let UMD_URL = Constants.UMD_URL;
        let index = UMD_URL.lastIndexOf('/');
        let str = UMD_URL.substring(index + 1, UMD_URL.length);
        let param = str.indexOf('?') != -1 ? '&_udId=' : '?_udId=';
        window.open(UMD_URL + param + designId, '_blank');
    };
    onAuthManageClick = (event, designId) => {
        ...
        axiosConfig.get(`/umd/config/authList/${designId}`)
            .then((result) => {
                if (result.data.data) {
    ...
    render() {
        let listUi = '', pageUi = '', headUi = '', searchUi = '', ui = '';
        ...
        if (this.state.pageDesign && this.state.pageDesign.length > 0) {
            listUi = this.state.pageDesign.map((item, index) => {
                ...
                return (
                    <div className="amp-list-row" key={index} data-id={item.designId}>
                        ...
                        <div
                            className="amp-list-cell-col col-type">{item.designType != 'null' ? item.designType : 'UMD'}</div>
            ...
            listUi = <div className="amp-list-null">没有查询到符合条件的设计模型！</div>;
        }
